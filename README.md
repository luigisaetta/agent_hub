...
| 22 | Audio mood classification (Gemini) | [`examples/example25.py`](examples/example25.py) | Reads the first local WAV file from `input_wav/` and classifies the mood into a predefined label set. | `responses.create(...)` with multimodal `input_text` + `input_audio` | Audio understanding/classification workflows. | Uses fixed model `google.gemini-2.5-pro` and no `system` role. |
